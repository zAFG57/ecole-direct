...
class EcoleDirect:
    ...
    def getHWj(self,jours):
        """get homework by days from the api"""
        if self.token is None or self.id is None:
            print("Error connection must be acitvate")
            return   

        connection = self.__req('https://api.ecoledirecte.com/v3/Eleves/{}/cahierdetexte/{}.awp?verbe=get&'.format(self.id,jours), """data={}"token":"{}"{}""".format("{", self.token, "}"))
        if connection.json()['code'] != 200:
            print("error ! bad username or password")
        else:
            # self.token = connection.json()['token']
    ...
    def getSL(self):
        ...
        if connection.json()['code'] != 200:
            ...
            return connection.json()['data']
    def getmes(self):
        """get message from the API"""
        if self.token is None or self.id is None:
            print("Error connection must be acitvate")
            return 
            
        connection = self.__req('https://api.ecoledirecte.com/v3/eleves/{}/messages.awp?verbe=getall&typeRecuperation=received&orderBy=date&order=desc&page=0&itemsPerPage=20&onlyRead=&query=&idClasseur=0'.format(self.id), """data={}"token":"{}"{}""".format("{", self.token, "}"))
        if connection.json()['code'] != 200:
            print("error !! bad username or password")
        else:
            # self.token = connection.json()['token']
